...
- `npx`を使用して`dorita980`パッケージを自動ダウンロード・実行
...
**注意：**
- このスクリプトを実行する前に、必ず`npm install`で依存関係をインストールしてください
...
**2. Roombaが起動していない（スリープモード）**

- Roomba本体の「CLEAN」ボタンを押して、Roombaを起動してください
- ホームベース上でも、スリープモードだと接続できないことがあります
