...
コードを実装・変更した後は、必ず以下のセルフレビュー・セルフ修正サイクルを実行すること。

1. 実装した内容をコードレビューする
2. コードレビューで挙がった各指摘事項の妥当性を評価する
3. 妥当性に基づき、修正計画を立てる
4. 修正すべきものに限り、修正を行う
5. 再度コードレビューする
6. 修正が妥当な指摘事項がなくなるまでこのプロセスを繰り返す

詳細な手順・レビュー観点・妥当性の評価基準は `review.instructions.md` を参照すること。
